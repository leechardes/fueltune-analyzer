# =============================================
# FuelTune Streamlit - IAM Resources
# =============================================
...
# =============================================
resource "aws_iam_role" "fueltune_app" {
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:fueltune:fueltune-service-account"
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_iam_role_policy" "fueltune_app" {
  name = "${local.name_prefix}-app-policy"
  role = aws_iam_role.fueltune_app.id
