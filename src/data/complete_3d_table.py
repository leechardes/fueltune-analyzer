...
def complete_3d_table(database_url: str = "sqlite:///data/fueltech_data.db"):
    ...
    with engine.connect() as connection:
        for column_name in columns_to_create:
            ...
            comment = f'Célula [{x},{y}]'
            
            try:
                # Criar a coluna
                alter_statement = text(
                    f"ALTER TABLE map_data_3d ADD COLUMN {column_name} FLOAT"
                )
...
def verify_3d_table_structure(database_url: str = "sqlite:///data/fueltech_data.db"):
    ...
    with engine.connect() as connection:
        ...
        if missing_columns:
            logger.warning(f"{len(missing_columns)} colunas faltando: {list(missing_columns)[:10]}...")
        else:
...
def create_sample_3d_data():
    ...
    for x, map_pressure in enumerate(map_values):
        ...
        for y, rpm in enumerate(rpm_values):
            ...
            # Cálculo final
            injection_time = base_value + (base_value * map_factor * 1.2) + (base_value * rpm_factor * 0.3)
