...
class DataSession(Base):
    ...
    vehicle_id = Column(
        String(36), ForeignKey("vehicles.id"), nullable=True, comment="ID do veículo vinculado"
    )
    ...
    # extended_data relationship removed after schema unification
...
class FuelTechCoreData(Base):
    ...
    # Unified extended fields (previously in fueltech_extended_data)
    ...
    __table_args__ = (
        ...
        # Constraints for unified extended fields (ported from former extended table)
        CheckConstraint(
            "estimated_power IS NULL OR (estimated_power >= 0 AND estimated_power <= 2000)",
            name="chk_power_range",
        ),
        CheckConstraint(
            "estimated_torque IS NULL OR (estimated_torque >= 0 AND estimated_torque <= 5000)",
            name="chk_torque_range",
        ),
        CheckConstraint(
            "g_force_accel IS NULL OR (g_force_accel >= -7.0 AND g_force_accel <= 7.0)",
            name="chk_g_accel_range",
        ),
        CheckConstraint(
            "g_force_lateral IS NULL OR (g_force_lateral >= -7.0 AND g_force_lateral <= 7.0)",
            name="chk_g_lateral_range",
        ),
    )


class FuelTechExtendedData:  # kept as placeholder for backward-compat imports
    pass
...
class Vehicle(Base):
    ...
    bank_a_mode = Column(
        String(20),
        default="semissequencial",
        comment="Modo: multiponto, semissequencial, sequencial",
    )
    ...
    updated_at = Column(
        DateTime, default=func.now(), onupdate=func.now(), comment="Última atualização"
    )
...
class DatabaseManager:
    ...
    def bulk_insert_core_data(self, session_id: str, data_records: List[Dict]) -> None:
        ...
        try:
            ...
            db.commit()
            logger.info(
                f"Bulk inserted {len(cleaned_records)} core data records (skipped {skipped_count} invalid)"
            )
    ...
    def bulk_insert_extended_data(self, session_id: str, data_records: List[Dict]) -> None:
        """Deprecated: extended data unified into FuelTechCoreData. No-op for compatibility."""
        logger.warning(
            "bulk_insert_extended_data is deprecated after schema unification; ignoring call."
        )
