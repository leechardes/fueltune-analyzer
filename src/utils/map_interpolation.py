...
class MapInterpolator:
    ...
    def interpolate_missing_points(self, x_values: List[float], y_values: List[float]) -> Tuple[List[float], List[float]]:
        ...
        valid_indices = [i for i, (x, y) in enumerate(zip(x_values, y_values)) 
                        if x is not None and y is not None and 
                        not (np.isnan(float(x)) or np.isnan(float(y)))]
        ...
        for i in range(len(x_values)):
            ...
            if (x_values[i] is not None and not np.isnan(float(x_values[i])) and
                (y_values[i] is None or np.isnan(float(y_values[i])))):
                try:
    ...
    def interpolate_3d_matrix(self, matrix: np.ndarray, method: str = 'linear') -> np.ndarray:
        ...
        try:
            interpolated_values = griddata(
                valid_points, valid_values, invalid_points,
    ...
    def convert_2d_maps_to_3d(self, map_2d_data: np.ndarray, rpm_2d_data: np.ndarray,
        ...
        try:
            # Validar entradas
            if (len(map_2d_data) != len(map_axis) or 
                len(rpm_2d_data) != len(rpm_axis)):
    ...
    def validate_interpolation_quality(self, x_values: List[float], y_values: List[float]) -> Dict[str, any]:
        """
        ...
        try:
            ...
            if not result["valid"]:
                result["issues"].append(f"Score de qualidade baixo: {result['quality_score']:.1f}/100")
