...
class MapValidator:
    ...
    def validate_2d_map(self, map_id: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            validations.append({
                "status": "INFO",
                "message": f"Validando mapa 2D {map_id}"
            })
    ...
    def validate_3d_map(self, map_id: str) -> List[Dict[str, str]]:
        ...
        try:
            validations.append({
                "status": "INFO",
                "message": f"Validando mapa 3D {map_id}"
            })
            ...
            validations.append({
                "status": "ERROR",
                "message": f"Erro na validação 3D: {str(e)}"
            })
            logger.error(f"Erro validando mapa 3D {map_id}: {str(e)}")
        
        return validations
    
    def validate_axis_data(self, axis_type: str, data_type: str, values: List[float]) -> List[Dict[str, str]]:
        """
    ...
    def _validate_map_structure(self, dimensions: int = 1) -> List[Dict[str, str]]:
        ...
        if dimensions == 1:
            validations.append({
                "status": "OK",
                "message": "Estrutura 2D válida"
            })
        elif dimensions == 2:
            validations.append({
                "status": "OK", 
                "message": "Estrutura 3D válida"
            })
        else:
            validations.append({
                "status": "ERROR",
                "message": f"Dimensão inválida: {dimensions}"
            })
        
        return validations
    
    def _validate_axis_data(self, axis_type: str, data_type: str, values: List[float]) -> List[Dict[str, str]]:
        ...
        if len(valid_values) < 2:
            validations.append({
                "status": "ERROR",
                "message": f"Eixo {axis_type}: menos de 2 valores válidos"
            })
            return validations
        
        # Verificar ordem crescente
        is_sorted = all(valid_values[i] <= valid_values[i+1] for i in range(len(valid_values)-1))
        
        if is_sorted:
            validations.append({
                "status": "OK",
                "message": f"Eixo {axis_type}: valores em ordem crescente"
            })
        else:
            validations.append({
                "status": "ERROR",
                "message": f"Eixo {axis_type}: valores fora de ordem"
            })
        ...
        if max_gap > 5 * avg_gap and avg_gap > 0:
            ...
            validations.append({
                "status": "OK",
                "message": f"Eixo {axis_type}: distribuição adequada"
            })
    ...
    def _validate_map_values(self, values: List[float], unit: str) -> List[Dict[str, str]]:
        ...
        if not valid_values:
            validations.append({
                "status": "ERROR",
                "message": "Nenhum valor válido encontrado"
            })
            return validations
        ...
        if unit in self.safety_limits:
            ...
            if min_val < limits["min"] or max_val > limits["max"]:
                ...
                validations.append({
                    "status": "OK",
                    "message": f"Valores dentro dos limites de segurança"
                })
        ...
        if value_range == 0:
            validations.append({
                "status": "WARNING",
                "message": "Todos os valores são idênticos - possível erro"
            })
        else:
            coefficient_variation = (np.std(valid_values) / mean_value) * 100 if mean_value != 0 else 0
    ...
    def _validate_3d_matrix_integrity(self) -> List[Dict[str, str]]:
        ...
        validations.append({
            "status": "OK",
            "message": "Matriz 3D: estrutura íntegra"
        })
        
        validations.append({
            "status": "OK", 
            "message": "Matriz 3D: 85% dos pontos com dados válidos"
        })
    ...
    def validate_map_consistency(self, map_a_data: Dict, map_b_data: Dict = None) -> List[Dict[str, str]]:
        """
        ...
        if not map_b_data:
            validations.append({
                "status": "INFO",
                "message": "Validação apenas da Bancada A (B não habilitada)"
            })
            return validations
        ...
        try:
            ...
            if (map_a_data.get("dimensions") != map_b_data.get("dimensions")):
                validations.append({
                    "status": "WARNING",
                    "message": "Bancadas têm dimensões diferentes (2D vs 3D)"
                })
            ...
            if (map_a_data.get("x_axis_type") != map_b_data.get("x_axis_type")):
                validations.append({
                    "status": "WARNING",
                    "message": "Bancadas têm tipos de eixo X diferentes"
                })
            ...
            if (map_a_data.get("data_unit") != map_b_data.get("data_unit")):
                validations.append({
                    "status": "ERROR",
                    "message": "Bancadas têm unidades de dados diferentes"
                })
            else:
                validations.append({
                    "status": "OK",
                    "message": "Bancadas têm estrutura compatível"
                })
    ...
    def validate_map_safety(self, map_data: Dict, engine_config: Dict) -> List[Dict[str, str]]:
        ...
        try:
            ...
            validations.append({
                "status": "OK",
                "message": "Validações de segurança básicas aprovadas"
            })
            
        except Exception as e:
            validations.append({
                "status": "ERROR",
                "message": f"Erro na validação de segurança: {str(e)}"
            })
    ...
    def generate_validation_report(self, validations: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        if report["warnings"] > 0:
            report["recommendations"].append(
                "Revise os avisos para otimizar a configuração"
            )
        
        if report["passed"] == report["total_checks"]:
            report["recommendations"].append(
                "Configuração aprovada em todas as verificações"
            )
