...
def calculate_duty_cycle(injection_time_ms: float, rpm: int, cylinders: int = 4, 
                        injection_mode: str = "sequencial") -> float:
    """
...
def estimate_power_per_bank(total_flow_lb_h: float, bsfc: float = 0.5, 
...
def check_bank_balance(bank_a_flow: float, bank_b_flow: float, 
...
def recommend_injector_size(target_power_hp: float, cylinders: int = 4, 
                           max_duty: float = 85.0, bsfc: float = 0.5,
                           num_banks: int = 1) -> Dict[str, float]:
    """
    ...
    common_sizes = [
        19, 24, 30, 36, 42, 47, 55, 60, 72, 80, 96, 110, 120, 160, 200, 250, 
        300, 370, 440, 550, 650, 750, 850, 1000, 1200, 1600, 2000
    ]
    
    # Encontrar tamanho mais próximo (maior que necessário para margem)
    recommended_size = min([size for size in common_sizes if size >= flow_per_injector], 
                          default=common_sizes[-1])
...
def calculate_injector_dead_time_compensation(voltage: float, injector_type: str = "standard") -> float:
    """
...
def validate_bank_configuration(bank_config: Dict[str, any]) -> List[Dict[str, str]]:
    ...
    validations.append({
        ...
        "message": f"Vazão por bico: {message}"
    })
    ...
    if dead_time < 0.1 or dead_time > 5.0:
        validations.append({
            "status": "WARNING", 
            "message": f"Dead time fora da faixa normal: {dead_time}ms"
        })
    else:
...
def compare_bank_configurations(bank_a: Dict[str, any], bank_b: Dict[str, any]) -> List[Dict[str, str]]:
    """
    ...
    # Comparar vazões
    flow_a = calculate_total_flow(
        bank_a.get("injector_flow", 0), 
        bank_a.get("injector_count", 0)
    )
    flow_b = calculate_total_flow(
        bank_b.get("injector_flow", 0), 
        bank_b.get("injector_count", 0)
    )
    
    balanced, balance_msg = check_bank_balance(flow_a, flow_b)
    comparisons.append({
        "status": "OK" if balanced else "WARNING",
        "message": f"Balanceamento: {balance_msg}"
    })
    ...
    if bank_a.get("mode") == bank_b.get("mode"):
        comparisons.append({
            "status": "OK", 
            "message": f"Modos idênticos: {bank_a.get('mode')}"
        })
    else:
        comparisons.append({
            "status": "INFO",
    ...
    if conflicts:
        comparisons.append({
            "status": "ERROR",
            "message": f"Conflitos de saída: {list(conflicts)}"
        })
    else:
        comparisons.append({
            "status": "OK",
            "message": "Nenhum conflito de saída"
        })
    ...
    if dead_time_diff > 0.5:  # Diferença > 0.5ms
        comparisons.append({
            "status": "WARNING",
            "message": f"Dead times muito diferentes: {dead_time_diff:.2f}ms"
        })
    else:
        comparisons.append({
            "status": "OK",
            "message": f"Dead times similares (diff: {dead_time_diff:.2f}ms)"
        })
    ...
    comparisons.append({
        "status": "INFO",
        "message": f"Potência total estimada: {total_power:.0f} HP"
    })
