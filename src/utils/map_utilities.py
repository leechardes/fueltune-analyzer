...
class FuelMapManager:
    ...
    def create_default_maps_for_vehicle(
        self, vehicle_id: str, bank_enabled_b: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        results = {"success": 0, "errors": 0, "created_maps": [], "error_details": []}
        ...
        default_maps_config = [
            ...
            {"template": create_default_rpm_compensation_map, "banks": [None], "shared": True},
            {"template": create_default_temp_compensation_map, "banks": [None], "shared": True},
            # Mapas adicionais
            {"template": self._create_tps_compensation_map, "banks": [None], "shared": True},
            {
                "template": self._create_voltage_compensation_map,
                "banks": ["A", "B"] if bank_enabled_b else ["A"],
                "shared": False,
            },
            {"template": self._create_cranking_pulse_map, "banks": [None], "shared": True},
        ]
        ...
        try:
            ...
            db_session.commit()
            logger.info(
                f"Mapas criados com sucesso: {results['success']}, Erros: {results['errors']}"
            )
    ...
    def _create_tps_compensation_map(self, vehicle_id: str, bank_id: str = None) -> Dict[str, Any]:
        ...
        tps_values = [0.00, 10.00, 20.00, 30.00, 40.00, 50.00, 60.00, 70.00, 80.00, 90.00, 100.00]

        # Valores de compensação (empobrecimento em carga baixa)
        compensation_values = [-5.0, -4.4, -3.8, -3.1, -2.5, -1.9, -1.3, -0.6, 0.0, 0.0, 0.0]
        ...
        return {
            ...
            "axis_data": {"X": {"data_type": "TPS", "active_slots": 11, "values": tps_values}},
            "values_data": compensation_values,
        }

    def _create_voltage_compensation_map(
        self, vehicle_id: str, bank_id: str = "A"
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "axis_data": {
                "X": {"data_type": "VOLTAGE", "active_slots": 9, "values": voltage_values}
            },
            "values_data": compensation_values,
        }
    ...
    def _create_cranking_pulse_map(self, vehicle_id: str, bank_id: str = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "axis_data": {"X": {"data_type": "TEMP", "active_slots": 8, "values": temp_values}},
    ...
    def duplicate_map_for_bank(self, source_map_id: str, target_bank_id: str) -> Optional[str]:
        ...
        try:
            ...
            source_axes = (
                db_session.query(MapAxisData).filter(MapAxisData.map_id == source_map_id).all()
            )
            ...
            if source_map.dimensions == 1:  # 2D
                source_data = (
                    db_session.query(MapData2D).filter(MapData2D.map_id == source_map_id).first()
                )
                if source_data:
                ...
                source_data = (
                    db_session.query(MapData3D).filter(MapData3D.map_id == source_map_id).first()
                )
                if source_data:
    ...
    def get_maps_summary_for_vehicle(self, vehicle_id: str) -> Dict[str, Any]:
        ...
        try:
            maps = (
                db_session.query(FuelMap)
                .filter(FuelMap.vehicle_id == vehicle_id, FuelMap.is_active == True)
                .all()
            )
            ...
            for map_obj in maps:
                ...
                # Última modificação
                if (
                    not summary["latest_modification"]
                    or map_obj.modified_at > summary["latest_modification"]
                ):
    ...
    def validate_map_data(self, map_id: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if fuel_map.x_slots_active <= 0:
                ...
                validations.append(
                    {"status": "OK", "message": f"Eixo X: {fuel_map.x_slots_active} slots ativos"}
                )

            if fuel_map.dimensions == 2 and fuel_map.y_slots_active <= 0:
                validations.append(
                    {"status": "ERROR", "message": "Mapa 3D sem slots ativos no eixo Y"}
                )
            elif fuel_map.dimensions == 2:
                validations.append(
                    {"status": "OK", "message": f"Eixo Y: {fuel_map.y_slots_active} slots ativos"}
                )
            ...
            if fuel_map.dimensions == 1:
                ...
                if data_2d:
                    valid_count = sum(
                        1
                        for i in range(fuel_map.x_slots_active)
                        if getattr(data_2d, f"value_{i}", None) is not None
                    )

                    if valid_count >= fuel_map.x_slots_active * 0.8:  # 80% dos valores
                        validations.append(
                            {"status": "OK", "message": f"Dados 2D: {valid_count} valores válidos"}
                        )
                    else:
                        validations.append(
                            {
                                "status": "WARNING",
                                "message": f"Dados 2D: apenas {valid_count} valores válidos",
                            }
                        )
