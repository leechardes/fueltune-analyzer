...
from .optimizer import global_optimizer
...
class EnhancedCacheManager:
    ...
    def set_with_optimization(
        ...
        try:
            ...
            # Use appropriate cache method based on type
            if cache_type == "dataframe":
                self.base_cache.set_dataframe(
                    session_id, operation, optimized_data, parameters, ttl
                )
            elif cache_type == "analysis":
                self.base_cache.set_analysis_result(
                    session_id, operation, optimized_data, parameters, ttl
                )
            elif cache_type == "chart":
                self.base_cache.set_chart_data(
                    session_id, operation, optimized_data, parameters, ttl
                )
            ...
            if cache_type == "dataframe":
                ...
                self.base_cache.set_analysis_result(
                    session_id, operation, data, parameters, ttl or 3600
                )
            elif cache_type == "chart":
                self.base_cache.set_chart_data(session_id, operation, data, parameters, ttl or 1800)

    def _apply_smart_strategies(
        self, cache_key: str, cache_type: str, data: Any, access_time: float
    ) -> None:
        ...
        try:
            ...
            if (
                access_count >= self.strategy_config["auto_promotion_threshold"]
                and cache_type == "dataframe"
                and access_time > 0.1
            ):  # Slow access indicates disk cache
            ...
            if (
                self.strategy_config["predictive_loading_enabled"] and access_count > 10
            ):  # High-usage item
            ...
            if len(key_parts) >= 3:
                ...
                if "filter" in operation:
                    ...
                    related_operations.extend(
                        [operation.replace("raw", "filtered"), operation.replace("raw", "stats")]
                    )
        ...
        try:
            report_data = {
                ...
                "access_patterns": {
                    ...
                    "access_distribution": dict(
                        sorted(self.access_counts.items(), key=lambda x: x[1], reverse=True)[:20]
                    ),  # Top 20 accessed keys
                    "avg_access_times": {
