...
class MapValidator:
        ...
        try:
            ...
            if values_matrix.size > 1:
                ...
                if coefficient_variation > 2.0:  # CV > 200%
                    warnings.append(f"Variação muito alta nos dados (CV: {coefficient_variation:.1%})")
    ...
    def validate_axis_values(
        ...
        try:
            ...
            if len(axis_values) != expected_size:
                errors.append(f"Eixo {axis_type} tem {len(axis_values)} valores, esperado {expected_size}")
    ...
    def validate_enabled_matrix(
        ...
        try:
            ...
            if len(enabled_matrix) != expected_size:
                errors.append(f"Matriz enabled tem {len(enabled_matrix)} linhas, esperado {expected_size}")
            
            for i, row in enumerate(enabled_matrix):
                if len(row) != expected_size:
                    errors.append(f"Linha {i} da matriz enabled tem {len(row)} colunas, esperado {expected_size}")
            ...
            if total_enabled < total_cells * 0.25:  # Menos de 25% habilitado
                logger.warning(f"Apenas {total_enabled}/{total_cells} células habilitadas ({total_enabled/total_cells:.1%})")
    ...
    def validate_complete_map(map_data: Map3DData) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if (len(map_data.data_matrix) != map_data.grid_size or
                len(map_data.enabled_matrix) != map_data.grid_size):
