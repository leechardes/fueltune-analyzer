...
class SessionManager:
    ...
    def cleanup_old_cache(max_age_minutes: int = 60):
        ...
        if current_vehicle:
            keys_to_remove = []
            for key in st.session_state.keys():
                if (key.startswith(SessionManager.MAP_DATA_PREFIX) and 
                    f"_{current_vehicle}_" not in key):
                    keys_to_remove.append(key)
            ...
            if keys_to_remove:
                logger.info(f"Cache limpo: {len(keys_to_remove)} mapas de outros veículos removidos")
