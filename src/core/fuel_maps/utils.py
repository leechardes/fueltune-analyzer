...
def find_matrix_peaks(matrix: np.ndarray, threshold_percentile: int = 90) -> List[Tuple[int, int, float]]:
    ...
    try:
        ...
        for i in range(half_kernel, rows - half_kernel):
            for j in range(half_kernel, cols - half_kernel):
                # Extrair janela
                window = matrix[i - half_kernel:i + half_kernel + 1, 
...
def export_matrix_to_csv(matrix: np.ndarray, filename: str, 
                        row_labels: Optional[List[str]] = None,
                        col_labels: Optional[List[str]] = None) -> bool:
