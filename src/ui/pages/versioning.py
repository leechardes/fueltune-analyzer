...
try:
    ...
    from ...maps.snapshots import MapSnapshots, SnapshotMetadata, SnapshotDiff
    from ...maps.editor import MapMetadata
    ...
    from src.maps.snapshots import MapSnapshots, SnapshotMetadata, SnapshotDiff
    from src.maps.editor import MapMetadata  
...
class VersioningPage:
    ...
    def _render_snapshot_history(self) -> None:
        ...
        with col2:
            map_type_filter = st.selectbox(
                "Tipo de mapa:",
                options=["Todos", "fuel", "ignition", "boost"],
                key="type_filter"
            )
        ...
        try:
            snapshots = self._get_filtered_snapshots(
                map_name_filter, map_type_filter, limit
            )
    ...
    def _render_ab_comparison(self) -> None:
        ...
        snapshot_options = [
            f"{s.map_name} v{s.version} ({s.created_at.strftime('%d/%m %H:%M')})"
            for s in snapshots
        ]
    ...
    def _render_storage_management(self) -> None:
        ...
        try:
            ...
            with col2:
                st.metric(
                    "Tamanho Comprimido",
                    self._format_file_size(stats['total_compressed_size'])
                )
            
            with col3:
                st.metric(
                    "Arquivo DB",
                    self._format_file_size(stats['database_file_size'])
                )
            
            with col4:
                st.metric(
                    "Tamanho Médio",
                    self._format_file_size(stats['average_snapshot_size'])
                )
            ...
            if stats['snapshots_by_type']:
                ...
                type_df = pd.DataFrame(
                    list(stats['snapshots_by_type'].items()),
            ...
            if stats['recent_activity']:
                ...
                activity_df = pd.DataFrame(
                    list(stats['recent_activity'].items()),
                    columns=['Data', 'Snapshots']
                )
                ...
                fig = px.line(
                    ...
                    x='Data',
                    y='Snapshots',
                    title="Snapshots Criados por Dia"
                )
                st.plotly_chart(fig, use_container_width=True)
            ...
            with col1:
                keep_count = st.number_input(
                    "Manter quantos snapshots por mapa:",
                    min_value=1,
                    max_value=50,
                    value=10
                )
    ...
    def _get_filtered_snapshots(
        self,
        map_name_filter: Optional[str],
        map_type_filter: Optional[str], 
        limit: int
    ...
    def _confirm_rollback(self, snapshot: SnapshotMetadata) -> None:
        ...
        with st.form(f"rollback_form_{snapshot.snapshot_id}"):
            st.warning(f"Confirmar rollback para snapshot **{snapshot.map_name} v{snapshot.version}**?")
    ...
    def _execute_rollback(self, snapshot: SnapshotMetadata) -> None:
        ...
        try:
            with st.spinner("Executando rollback..."):
                map_data, metadata = self.snapshots.rollback_to_snapshot(
    ...
    def _cleanup_old_snapshots(self, keep_count: int) -> None:
        ...
        try:
            with st.spinner("Limpando snapshots antigos..."):
                deleted_count = self.snapshots.cleanup_old_snapshots(
...
def main():
    ...
    # Configure page
    st.set_page_config(
        page_title="Versionamento - FuelTune",
