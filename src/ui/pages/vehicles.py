...
from src.data.vehicle_validators import normalize_plate, validate_vehicle_data
...
# Abas principais
tab_list, tab_new, tab_edit = st.tabs(["Lista de Veículos", "Cadastrar Novo", "Editar Veículo"])
    ...
    if not vehicles:
        ...
        for vehicle in vehicles:
            with st.container():
                ...
                with st.expander(
                    f"{status_icon} **{vehicle['name']}** ({vehicle.get('nickname', 'Sem apelido')})"
                ):
                    ...
                    with col1:
                        if st.button(
                            "Ver Detalhes", key=f"view_{vehicle['id']}", use_container_width=True
                        ):
                    ...
                    with col2:
                        if st.button(
                            "Editar", key=f"edit_{vehicle['id']}", use_container_width=True
                        ):
    ...
    with st.form("new_vehicle_form"):
        ...
        with col1:
            engine_displacement = st.number_input(
                "Cilindrada (L)", min_value=0.0, max_value=10.0, value=2.0, step=0.1
            )
            engine_cylinders = st.number_input("Cilindros", min_value=1, max_value=16, value=4)

        with col2:
            engine_configuration = st.selectbox(
                "Configuração", ["I4", "I6", "V6", "V8", "V10", "V12", "H4", "H6"]
            )
            engine_aspiration = st.selectbox(
                "Aspiração", ["Aspirado", "Turbo", "Supercharger", "Turbo + Supercharger"]
            )

        with col3:
            estimated_power = st.number_input(
                "Potência (CV)", min_value=0, max_value=2600, value=200
            )
            estimated_torque = st.number_input(
                "Torque (Nm)", min_value=0, max_value=2000, value=300
            )

        with col4:
            max_rpm = st.number_input(
                "RPM Máximo", min_value=1000, max_value=20000, value=7000, step=100
            )
            fuel_type = st.selectbox(
                "Combustível", ["Gasolina", "Etanol", "Flex", "Diesel", "GNV", "E85"]
            )
        ...
        with col1:
            ...
            injector_count = st.number_input(
                "Quantidade de Bicos", min_value=1, max_value=16, value=4
            )

        with col2:
            injector_flow_rate = st.number_input(
                "Vazão (lb/h)", min_value=0, max_value=300, value=80
            )
            fuel_rail_pressure = st.number_input(
                "Pressão da Linha (bar)", min_value=0.0, max_value=10.0, value=3.5, step=0.1
            )

        with col3:
            fuel_system = st.selectbox(
                "Sistema", ["Port Injection", "Direct Injection", "Dual Injection"]
            )
        ...
        if engine_aspiration in ["Turbo", "Turbo + Supercharger"]:
            ...
            with col2:
                max_boost_pressure = st.number_input(
                    "Pressão Máxima (bar)", min_value=0.0, max_value=5.0, value=1.5, step=0.1
                )
                wastegate_type = st.selectbox(
                    "Tipo de Wastegate", ["Interna", "Externa", "Eletrônica"]
                )
        ...
        with col1:
            transmission_type = st.selectbox(
                "Transmissão", ["Manual", "Automática", "DSG/DCT", "CVT"]
            )
            gear_count = st.number_input("Marchas", min_value=3, max_value=10, value=6)

        with col2:
            drivetrain = st.selectbox("Tração", ["FWD", "RWD", "AWD", "4WD"])
            final_drive_ratio = st.number_input(
                "Relação Final", min_value=2.0, max_value=6.0, value=3.5, step=0.01
            )
        ...
        with col1:
            ...
            if bank_a_enabled:
                bank_a_mode = st.selectbox(
                    "Modo Bancada A", ["sequential", "semi-sequential", "batch"], key="bank_a_mode"
                )
                bank_a_injector_count = st.number_input(
                    "Qtd Bicos A", min_value=1, max_value=8, value=4, key="bank_a_count"
                )
                bank_a_injector_flow = st.number_input(
                    "Vazão Bicos A (lb/h)", min_value=0, max_value=300, value=80, key="bank_a_flow"
                )
        ...
        with col2:
            ...
            if bank_b_enabled:
                bank_b_mode = st.selectbox(
                    "Modo Bancada B", ["sequential", "semi-sequential", "batch"], key="bank_b_mode"
                )
                bank_b_injector_count = st.number_input(
                    "Qtd Bicos B", min_value=1, max_value=8, value=4, key="bank_b_count"
                )
                bank_b_injector_flow = st.number_input(
                    "Vazão Bicos B (lb/h)", min_value=0, max_value=300, value=80, key="bank_b_flow"
                )
        ...
        with col1:
            min_map_pressure = st.number_input(
                "Pressão MAP Mínima (kPa)", min_value=0.0, max_value=300.0, value=0.0, step=1.0
            )

        with col2:
            max_map_pressure = st.number_input(
                "Pressão MAP Máxima (kPa)", min_value=0.0, max_value=500.0, value=250.0, step=1.0
            )
        ...
        submitted = st.form_submit_button(
            "Cadastrar Veículo", type="primary", use_container_width=True
        )
        ...
        if submitted:
            ...
            vehicle_data = {
                    ...
                    if fuel_type == "Etanol"
                        ...
                        if fuel_type == "Gasolina"
                            ...
                            if fuel_type == "Flex"
                                ...
                                "Diesel"
                                if fuel_type == "Diesel"
                                else "CNG" if fuel_type == "GNV" else fuel_type
                            )
                        )
                    )
                ),
                ...
                "bank_a_total_flow": (
                    bank_a_injector_flow * bank_a_injector_count if bank_a_enabled else 0
                ),
                ...
                "bank_b_total_flow": (
                    bank_b_injector_flow * bank_b_injector_count if bank_b_enabled else 0
                ),
                ...
                with st.form("edit_vehicle_form"):
                    ...
                    with col3:
                        year = st.number_input(
                            "Ano", min_value=1900, max_value=2030, value=vehicle.get("year", 2020)
                        )
