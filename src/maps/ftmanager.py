...
class FTManagerBridge:
    ...
    def import_from_clipboard(
        self, expected_format: Optional[FTManagerFormat] = None, validate_dimensions: bool = True
    ) -> ImportResult:
        """
            ...
            if update_clipboard:
                ...
                if not clipboard_success:
                    format_warnings.append(
                        "Failed to update clipboard, but data formatted successfully"
                    )
    ...
    def validate_ftmanager_data(
        self, data_content: str, expected_format: Optional[FTManagerFormat] = None
            ...
            try:
                ...
                if len(numeric_cols) > 0:
                    for col in numeric_cols:
                        ...
                        if any(
                            len(str(val).split(".")[-1]) > 6 for val in col_data if "." in str(val)
                        ):
    ...
    def create_custom_format(
        ...
        try:
            ...
            logger.debug(
                f"Created custom format '{name}': {dimensions} with separator '{separator}'"
            )
    ...
    def _detect_format(self, content: str) -> Optional[FTManagerFormat]:
        ...
        try:
            ...
            logger.debug(
                f"Detected format: {format_type} {dimensions} with separator '{separator}'"
            )
    ...
    def _parse_data(
        self, content: str, format_spec: FTManagerFormat
    ...
    def _validate_dimensions(
        self, map_data: pd.DataFrame, format_spec: FTManagerFormat
    ) -> List[str]:
        ...
        if actual_dims != expected_dims:
            warnings.append(f"Dimension mismatch: got {actual_dims}, expected {expected_dims}")

        return warnings

    def _formats_compatible(self, format1: FTManagerFormat, format2: FTManagerFormat) -> bool:
        """Check if two formats are compatible."""

        return (
            format1.format_type == format2.format_type
            and format1.separator == format2.separator
            and abs(format1.decimal_places - format2.decimal_places) <= 1
        )
    ...
    def _validate_export_compatibility(
        self, map_data: pd.DataFrame, target_format: FTManagerFormat
    ) -> List[str]:
        ...
        try:
            ...
            if target_format.format_type == "csv":
                ...
                formatted_data = hex_data.to_csv(
                    sep=target_format.separator, header=target_format.has_headers, index=False
                )
