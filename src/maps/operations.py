...
class MapOperations:
    ...
    def increment_region(
        ...
        try:
            ...
            logger.debug(f"Incremented region by {increment_value}, affected {old_values.size} cells")
    ...
    def copy_region(
        ...
        try:
            ...
            logger.debug(f"Copied region {selection} to clipboard, shape: {self.clipboard_data.shape}")
    ...
    def paste_region(
        self,
        map_data: pd.DataFrame,
        target_selection: CellSelection,
    ...
    def _validate_selection(self, map_data: pd.DataFrame, selection: CellSelection) -> None:
        ...
        if selection.start_row < 0 or selection.end_row > max_row:
            raise ValueError(f"Row selection out of bounds: {selection.start_row}-{selection.end_row}")
        
        if selection.start_col < 0 or selection.end_col > max_col:
            raise ValueError(f"Column selection out of bounds: {selection.start_col}-{selection.end_col}")
