...
class MapEditor:
    ...
    def _render_map_info_panel(self) -> None:
        ...
        increment_value = st.number_input(
            "Increment/Decrement Value", value=0.1, step=0.1, format="%.2f"
        )
    ...
    def _render_status_bar(self) -> None:
        ...
        with col1:
            if st.session_state.current_map is not None:
                ...
                st.markdown(
                    f'<span class="map-info">Modified cells: {cells_modified}</span>',
                    unsafe_allow_html=True,
                )
        ...
        with col2:
            if st.session_state.map_metadata:
                ...
                st.markdown(
                    f'<span class="map-info">Last modified: {last_modified}</span>',
                    unsafe_allow_html=True,
                )
    ...
    def _create_new_map(
        self, name: str, map_type: str, dimensions: Tuple[int, int], rpm_range: Tuple[int, int]
    ) -> None:
        """
        ...
        try:
            if st.session_state.current_map is not None:
                self.snapshots.save_snapshot(
                    st.session_state.current_map, st.session_state.map_metadata
                )
        ...
        try:
            if st.session_state.current_map is not None:
                smoothed_map = self.algorithms.gaussian_smooth(
                    st.session_state.current_map, sigma=1.0
                )
    ...
    def _increment_selected_cells(self, value: float) -> None:
        ...
        try:
            if st.session_state.current_map is not None:
                ...
                numeric_columns = st.session_state.current_map.select_dtypes(
                    include=[np.number]
                ).columns
