...
class MapAlgorithms:
            ...
            for col in numeric_cols:
                ...
                if preserve_edges:
                    ...
                    smoothed = ndimage.gaussian_filter(
                        values, sigma=sigma, mode="reflect"  # Reflect at boundaries
                    )
    ...
    def savitzky_golay_smooth(
        self, map_data: pd.DataFrame, window_length: int = 5, polyorder: int = 2, axis: int = 0
    ) -> pd.DataFrame:
        """
            ...
            for col in numeric_cols:
                ...
                try:
                    smoothed = savgol_filter(
                        values, window_length, polyorder, axis=axis, mode="nearest"
                    )
    ...
    def bilateral_filter(
        self, map_data: pd.DataFrame, sigma_spatial: float = 1.0, sigma_range: float = 0.1
    ) -> pd.DataFrame:
        """
    ...
    def interpolate_missing(
        self, map_data: pd.DataFrame, method: str = "cubic", fill_value: Optional[float] = None
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            for col in numeric_cols:
                ...
                if method in ["linear", "cubic", "nearest"]:
                    ...
                    interpolated = self._rbf_interpolation(values, valid_mask, fill_value)
                else:
    ...
    def detect_and_correct_outliers(
        ...
        try:
            ...
            self.last_operation_time = time.time() - start_time
            logger.debug(
                f"Outlier detection/correction completed in {self.last_operation_time:.3f}s"
            )
    ...
    def adaptive_smooth(
        self, map_data: pd.DataFrame, noise_threshold: float = 0.1, max_sigma: float = 2.0
    ) -> pd.DataFrame:
        """
    ...
    def _edge_preserving_gaussian(
        self, values: np.ndarray, sigma: float, mask: np.ndarray
    ) -> np.ndarray:
        ...
        gradient_mag = np.sqrt(
            ndimage.sobel(values, axis=0) ** 2 + ndimage.sobel(values, axis=1) ** 2
        )
    ...
    def _bilateral_filter_2d(
        self, values: np.ndarray, sigma_spatial: float, sigma_range: float
    ) -> np.ndarray:
    ...
    def _griddata_interpolation(
        self, values: np.ndarray, valid_mask: np.ndarray, method: str, fill_value: Optional[float]
    ) -> np.ndarray:
        ...
        valid_points = np.column_stack(
            [rows.flatten()[valid_mask.flatten()], cols.flatten()[valid_mask.flatten()]]
        )
    ...
    def _rbf_interpolation(
        self, values: np.ndarray, valid_mask: np.ndarray, fill_value: Optional[float]
    ) -> np.ndarray:
    ...
    def _estimate_local_noise(self, values: np.ndarray, window_size: int = 3) -> np.ndarray:
        ...
        local_std = ndimage.generic_filter(values, np.nanstd, size=window_size, mode="reflect")
