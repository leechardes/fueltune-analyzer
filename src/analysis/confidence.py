...
class ConfidenceScorer:
    ...
    def _generate_confidence_recommendations(
        ...
        if quantity_score < 0.6:
            improvements.append(
                f"Collect more data points (currently {quality_metrics.total_points}, recommend >500)"
            )

        if quality_score < 0.6:
            improvements.append(
                "Improve data quality by addressing sensor calibration or filtering"
            )

        if DataQualityIssue.MISSING_VALUES in quality_metrics.identified_issues:
            improvements.append(
                f"Address missing values ({quality_metrics.missing_percentage:.1f}% missing)"
            )

        if DataQualityIssue.OUTLIERS_PRESENT in quality_metrics.identified_issues:
            improvements.append(
                f"Investigate outliers ({quality_metrics.outlier_percentage:.1f}% detected)"
            )
        ...
        if consistency_score < 0.6:
            improvements.append(
                "Improve measurement consistency through calibration or methodology"
            )
...
def assess_data_quality(
    data: Union[pd.DataFrame, Dict[str, np.ndarray]], config: Optional[ConfidenceConfig] = None
) -> DataQualityMetrics:
    """
    Comprehensive data quality assessment.
...
def validate_analysis_confidence(
    analysis_result: Any, min_confidence: float = 0.6
) -> Tuple[bool, List[str]]:
    """
    ...
    if not hasattr(analysis_result, "confidence_score") and not hasattr(
    ...
    if not is_valid:
        messages.append(
            f"Confidence score {confidence_score:.3f} below minimum required {min_confidence:.3f}"
        )
