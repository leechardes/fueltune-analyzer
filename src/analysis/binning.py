...
class AdaptiveBinner:
    ...
    def _validate_binning_data(self, arrays: Dict[str, np.ndarray]) -> None:
        ...
        if len(rpm) < self.config.min_points_per_bin:
            raise ValueError(
                f"Insufficient data points for binning (minimum {self.config.min_points_per_bin})"
            )
    ...
    def _determine_optimal_ranges(
        ...
        if self.config.auto_rpm_range:
            ...
            rpm_range = (max(0, rpm_min - rpm_padding), rpm_max + rpm_padding)
        else:
            rpm_range = (self.config.rpm_min or np.min(rpm), self.config.rpm_max or np.max(rpm))
        ...
        if self.config.auto_map_range:
            ...
            map_range = (max(0, map_min - map_padding), map_max + map_padding)
        else:
            map_range = (
    ...
    def _analyze_data_density(
        self, arrays: Dict[str, np.ndarray], base_grid: Dict[str, Any]
    ) -> np.ndarray:
        """
        ...
        density_map, _, _ = np.histogram2d(
            rpm, map_pressure, bins=[base_grid["rpm_edges"], base_grid["map_edges"]]
        )
    ...
    def _create_adaptive_bins(
        self, arrays: Dict[str, np.ndarray], base_grid: Dict[str, Any], density_map: np.ndarray
    ...
    def _calculate_parameter_statistics(self, data: np.ndarray) -> Dict[str, float]:
        ...
        return {
            ...
            "outlier_count": (
                int(len(data) - len(clean_data)) if len(clean_data) != len(data) else 0
            ),
        }
    ...
    def _validate_and_filter_bins(
        ...
        for bin_key, bin_cell in bins.items():
            ...
            if bin_cell.point_count > self.config.max_points_per_bin:
                logger.debug(
                    f"Bin {bin_key} has {bin_cell.point_count} points (>max {self.config.max_points_per_bin})"
                )
    ...
    def _calculate_overall_statistics(
        self, bins: Dict[Tuple[int, int], BinCell], arrays: Dict[str, np.ndarray]
    ) -> Dict[str, Any]:
        """
        ...
        stats = {
            ...
            "coverage_percentage": (
                (binned_points / total_points) * 100.0 if total_points > 0 else 0.0
            ),
...
def analyze_bin_density(
    binning_result: BinningResult, density_threshold: float = 0.5
) -> Dict[str, Any]:
    """
...
def calculate_bin_statistics(
    binning_result: BinningResult, parameter: str = "lambda_sensor"
