...
class SafetyValidator:
    ...
    def _validate_safety_data(self, arrays: Dict[str, np.ndarray]) -> None:
        ...
        for param in critical_params:
            ...
            if np.all(np.isnan(arrays[param])):
                raise ValueError(
                    f"Critical safety parameter '{param}' contains only invalid values"
                )
    ...
    def _validate_lambda_safety(
        ...
        if np.any(lean_violation_mask):
            self._process_lambda_violations(
                ...
                timestamps,
                lean_violation_mask,
                ViolationType.LEAN_CONDITION,
                self.config.warning_lambda_lean,
        ...
        if np.any(critical_lean_mask):
            self._process_lambda_violations(
                ...
                critical_lean_mask,
                ViolationType.LEAN_CONDITION,
                self.config.critical_lambda_lean,
        ...
        if np.any(critical_rich_mask):
            self._process_lambda_violations(
                ...
                critical_rich_mask,
                ViolationType.RICH_CONDITION,
                self.config.critical_lambda_rich,
    ...
    def _process_lambda_violations(
        ...
        for group in consecutive_groups:
            ...
            violation = SafetyViolation(
                ...
                duration=(
                    float(timestamps[end_idx] - timestamps[start_idx])
    ...
    def _validate_temperature_safety(self, arrays: Dict[str, np.ndarray]) -> List[SafetyViolation]:
        ...
        if "egt" in arrays and not np.all(np.isnan(arrays["egt"])):
            egt_violations = self._validate_temperature_parameter(
                ...
                self.config.max_egt_warning,
                self.config.max_egt_critical,
                ViolationType.HIGH_EGT,
                "egt",
                "EGT",
            )
        ...
        if "engine_temp" in arrays and not np.all(np.isnan(arrays["engine_temp"])):
            coolant_violations = self._validate_temperature_parameter(
                arrays["engine_temp"],
                arrays.get("timestamp"),
                self.config.max_coolant_warning,
                self.config.max_coolant_critical,
                ViolationType.COOLANT_TEMP_HIGH,
                "engine_temp",
                "Coolant",
            )
        ...
        if "intake_temp" in arrays and not np.all(np.isnan(arrays["intake_temp"])):
            intake_violations = self._validate_temperature_parameter(
                arrays["intake_temp"],
                arrays.get("timestamp"),
                self.config.max_intake_warning,
                self.config.max_intake_critical,
                ViolationType.INTAKE_TEMP_HIGH,
                "intake_temp",
                "Intake",
            )
    ...
    def _validate_temperature_parameter(
        ...
        clean_timestamps = (
            timestamps[valid_mask] if timestamps is not None else np.arange(len(clean_temps))
        )
        ...
        if np.any(warning_mask):
            violations.extend(
                self._create_temperature_violations(
                    clean_temps,
                    clean_timestamps,
                    warning_mask,
                    violation_type,
                    ...
                    warning_limit,
                    SafetyLevel.WARNING,
                    f"{display_name} temperature exceeds warning limit",
                )
            )
        ...
        if np.any(critical_mask):
            violations.extend(
                self._create_temperature_violations(
                    ...
                    f"CRITICAL: {display_name} temperature exceeds safe limit",
                )
            )
    ...
    def _validate_timing_safety(self, arrays: Dict[str, np.ndarray]) -> List[SafetyViolation]:
        ...
        warning_limits = np.where(
            is_boost, self.config.max_timing_boost_warning, self.config.max_timing_na_warning
        )
        critical_limits = np.where(
            is_boost, self.config.max_timing_boost_critical, self.config.max_timing_na_critical
        )
        ...
        if np.any(warning_mask):
            violations.extend(
                self._create_timing_violations(
                    clean_timing,
                    timestamps,
                    warning_mask,
                    is_boost,
                    SafetyLevel.WARNING,
                    "Ignition timing approaches maximum safe advance",
                )
            )
        ...
        if np.any(critical_mask):
            violations.extend(
                self._create_timing_violations(
                    ...
                    is_boost,
                    SafetyLevel.CRITICAL,
                    "CRITICAL: Excessive ignition timing advance",
                )
            )
    ...
    def _create_timing_violations(
        ...
        for group in consecutive_groups:
            ...
            safe_limit = (
                self.config.max_timing_boost_warning
                if is_boost_condition
                else self.config.max_timing_na_warning
            )
    ...
    def _validate_boost_safety(self, arrays: Dict[str, np.ndarray]) -> List[SafetyViolation]:
        ...
        if np.any(warning_mask):
            violations.extend(
                self._create_boost_violations(
                    clean_map,
                    timestamps,
                    warning_mask,
                    self.config.max_boost_warning,
                    SafetyLevel.WARNING,
                )
            )
        ...
        if np.any(critical_mask):
            violations.extend(
                self._create_boost_violations(
                    ...
                    timestamps,
                    critical_mask,
                    self.config.max_boost_critical,
                    SafetyLevel.CRITICAL,
                )
            )
    ...
    def _validate_rpm_safety(self, arrays: Dict[str, np.ndarray]) -> List[SafetyViolation]:
        ...
        if np.any(warning_mask):
            violations.extend(
                self._create_rpm_violations(
                    clean_rpm,
                    timestamps,
                    warning_mask,
                    self.config.max_rpm_warning,
                    SafetyLevel.WARNING,
                )
            )
    ...
    def _validate_fuel_safety(self, arrays: Dict[str, np.ndarray]) -> List[SafetyViolation]:
        ...
        if np.any(warning_mask):
            violations.extend(
                self._create_fuel_violations(
                    clean_pressure,
                    timestamps,
                    warning_mask,
                    self.config.min_fuel_pressure_warning,
                    SafetyLevel.WARNING,
                )
            )
        ...
        if np.any(critical_mask):
            violations.extend(
                self._create_fuel_violations(
                    ...
                    timestamps,
                    critical_mask,
                    self.config.min_fuel_pressure_critical,
                    SafetyLevel.CRITICAL,
                )
            )
    ...
    def _calculate_safety_metrics(
        self, result: SafetyResult, arrays: Dict[str, np.ndarray]
    ) -> None:
        """Calculate overall safety metrics."""
        ...
        total_violation_points = (
            result.warning_points + result.critical_points + result.emergency_points
        )
    ...
    def _generate_safety_recommendations(
            ...
            if violation_type == ViolationType.LEAN_CONDITION:
                if critical_violations:
                    result.immediate_actions.append(
                        "IMMEDIATE: Increase fuel delivery to prevent engine damage"
                    )
                result.safety_recommendations.append(
                    "Review and enrich fuel maps in affected areas"
                )

            elif violation_type == ViolationType.HIGH_EGT:
                if critical_violations:
                    result.immediate_actions.append(
                        "IMMEDIATE: Reduce engine load and check cooling"
                    )
                result.safety_recommendations.append(
                    "Improve engine cooling and reduce ignition advance"
                )
                ...
                result.safety_recommendations.append(
                    "Conservative timing approach with gradual optimization"
                )
        ...
        if result.overall_safety_level in [SafetyLevel.CRITICAL, SafetyLevel.EMERGENCY]:
            result.immediate_actions.insert(
                0, "STOP ENGINE OPERATION IMMEDIATELY - DAMAGE RISK DETECTED"
            )
            ...
            if violation.safety_level in [SafetyLevel.CRITICAL, SafetyLevel.EMERGENCY]:
                if violation.violation_type == ViolationType.LEAN_CONDITION:
                    constraints_applied.append(
                        f"Lambda limited to maximum {violation.safe_limit:.3f}"
                    )
                elif violation.violation_type == ViolationType.EXCESSIVE_TIMING:
                    constraints_applied.append(
                        f"Ignition timing limited to {violation.safe_limit:.1f}° BTDC"
                    )
                elif violation.violation_type == ViolationType.OVERBOOST:
                    constraints_applied.append(
                        f"Boost pressure limited to {violation.safe_limit:.2f} bar"
                    )

        if constraints_applied:
            logger.info(f"Applied safety constraints: {', '.join(constraints_applied)}")

    def _get_lambda_immediate_action(
        self, violation_type: ViolationType, safety_level: SafetyLevel
    ) -> str:
...
def check_critical_parameters(
    data: Union[pd.DataFrame, Dict[str, np.ndarray]], parameters: Optional[List[str]] = None
    ...
    for param in parameters:
        ...
        if param_violations:
            critical_violations = [
                v for v in param_violations if v.safety_level == SafetyLevel.CRITICAL
            ]
            param_status[param] = {
...
def apply_safety_constraints(
    ...
    safety_factor = (
        config.safety_factor_conservative if conservative else config.safety_factor_aggressive
    )
