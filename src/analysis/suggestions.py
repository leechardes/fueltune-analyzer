...
from .segmentation import SegmentationResult, EngineState
...
class TuningSuggestion:
    ...
    def __post_init__(self):
        ...
        if self.current_value != 0:
            self.adjustment_percentage = ((self.suggested_value - self.current_value) / self.current_value) * 100.0
...
class SuggestionEngine:
    ...
    def generate_suggestions(
        ...
        try:
            ...
            result = SuggestionsResult(
                ...
                critical_suggestions=len([s for s in ranked_suggestions if s.priority == SuggestionPriority.CRITICAL]),
    ...
    def _prepare_analysis_data(
        ...
        if isinstance(data, pd.DataFrame):
            ...
            arrays["throttle_position"] = data.get("throttle_position", pd.Series(dtype=float)).values.astype(np.float32)
            arrays["map_pressure"] = data.get("map_pressure", pd.Series(dtype=float)).values.astype(np.float32)
            arrays["lambda_sensor"] = data.get("lambda_sensor", pd.Series(dtype=float)).values.astype(np.float32)
    ...
    def _validate_analysis_data(self, arrays: Dict[str, np.ndarray]) -> None:
        ...
        if len(arrays["rpm"]) < self.config.min_points_for_suggestion:
            raise ValueError(f"Insufficient data points (minimum {self.config.min_points_for_suggestion})")
        ...
        if len(valid_lambda) < len(lambda_values) * 0.8:
            logger.warning("Lambda sensor data quality may be poor - suggestions may be less reliable")
    ...
    def _generate_fuel_suggestions(
        ...
        if segmentation_result:
            suggestions.extend(
                self._analyze_fuel_by_states(data, segmentation_result)
            )
        
        # Analyze by RPM bins if binning is available
        if binning_result:
            suggestions.extend(
                self._analyze_fuel_by_bins(data, binning_result)
            )
        
        # General fuel analysis
        suggestions.extend(
            self._analyze_general_fuel_trends(clean_lambda, clean_rpm)
        )
    ...
    def _generate_ignition_suggestions(
        ...
        if segmentation_result:
            suggestions.extend(
                self._analyze_timing_by_states(data, segmentation_result)
            )
    ...
    def _analyze_timing_by_states(
        ...
        map_data = data.get("map_pressure", np.ones_like(timing_data))
    ...
    def _generate_optimization_suggestions(
        ...
        high_confidence_bins = sum(1 for bin_cell in binning_result.bins.values() 
                                   if bin_cell.confidence_score > 0.8)
    ...
    def _calculate_suggestion_confidence(
        ...
        for suggestion in suggestions:
            ...
            if suggestion.current_value != 0:
                deviation = abs(suggestion.suggested_value - suggestion.current_value) / suggestion.current_value
    ...
    def _calculate_summary_statistics(
        ...
        return {
            ...
            "data_coverage": {
                ...
                "suggestions_with_high_confidence": sum(1 for s in suggestions if s.confidence_score > 0.8)
            }
        }
    ...
    def _create_suggestions_metadata(
        ...
        metadata = {
            ...
            "data_characteristics": {
                ...
                "lambda_range": [float(np.min(data["lambda_sensor"])), float(np.max(data["lambda_sensor"]))]
            }
        }
...
def rank_suggestions_by_priority(suggestions: List[TuningSuggestion]) -> List[TuningSuggestion]:
    ...
    return sorted(
        suggestions,
        key=lambda s: (s.priority.value, -s.confidence_score, -s.impact_score)
    )
