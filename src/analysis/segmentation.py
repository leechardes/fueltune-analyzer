...
class EngineStateSegmenter:
    ...
    def segment_data(
        ...
        try:
            ...
            arrays = self._prepare_data_arrays(
                data, timestamp_col, rpm_col, tps_col, 
                map_col, lambda_col, two_step_col, launch_col
            )
    ...
    def _classify_states_vectorized(
        ...
        state_masks[EngineState.LAUNCH] = (
            launch | 
        ...
        state_masks[EngineState.OVERRUN] = (
            (lambda_val >= self.config.overrun_lambda_min) & 
            (tps < 5.0) & 
            (rpm > 2000)
        )
        ...
        # Load-based states
        state_masks[EngineState.IDLE] = (
            (rpm <= self.config.idle_rpm_max) & 
        ...
        for state in [EngineState.IDLE, EngineState.LIGHT_LOAD, 
                     EngineState.MODERATE_LOAD, EngineState.HIGH_LOAD, EngineState.BOOST]:
            if state in state_masks:
    ...
    def _post_process_segments(
        self,
        state_classifications: Dict[EngineState, np.ndarray],
    ...
    def _calculate_segment_statistics(
        self,
        segments: Dict[EngineState, np.ndarray],
    ...
    def _create_metadata(self, arrays: Dict[str, np.ndarray]) -> Dict[str, Any]:
        ...
        return {
            ...
            "rpm_range": {
                "min": int(np.min(arrays["rpm"])),
                "max": int(np.max(arrays["rpm"]))
            },
            "config": {
...
def identify_operating_states(
    data: Union[pd.DataFrame, Dict[str, np.ndarray]],
    return_dominant_only: bool = True
