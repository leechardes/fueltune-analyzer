...
from src.utils.bank_calculations import (
    calculate_total_flow, validate_injector_flow, check_bank_balance,
    validate_bank_configuration, compare_bank_configurations,
    recommend_injector_size, calculate_injector_dead_time_compensation
)
...
class BankConfigurator:
    ...
    def _render_configuration_summary(self, vehicle: Vehicle) -> None:
        ...
        with col2:
            outputs_b_count = len(self._parse_outputs(vehicle.bank_b_outputs) or []) if vehicle.bank_b_enabled else 0
    ...
    def _render_action_buttons(self, vehicle: Vehicle) -> None:
        ...
        with col4:
            if st.button(":material/settings_backup_restore: Configuração Padrão", key="restore_default"):
    ...
    def _render_injector_calculator(self) -> None:
        ...
        if st.button(":material/calculate: Calcular Recomendação", key="calc_injector_size"):
            ...
            if "error" not in recommendation:
                ...
                with rec_col1:
                    st.metric("Injetor Recomendado", f"{recommendation['recommended_injector_size_lb_h']:.0f} lb/h")
                    st.metric("Injetores por Bancada", f"{recommendation['injectors_per_bank']}")
                    st.metric("Potência Máxima Real", f"{recommendation['actual_max_power_hp']:.0f} HP")
                
                with rec_col2:
                    st.metric("Vazão Total", f"{recommendation['actual_total_flow_lb_h']:.0f} lb/h")
                    st.metric("Margem de Segurança", f"{recommendation['safety_margin_percent']:.1f}%")
