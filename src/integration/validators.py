...
class FTManagerValidator:
        ...
        try:
            ...
            if any(issue.severity == 'error' for issue in basic_issues):
                return self._compile_result(
                    False, 0.0, issues, details, validation_start
                )
            ...
            return ValidationResult(
                ...
                performance_metrics={'validation_duration_ms': (datetime.now() - validation_start).total_seconds() * 1000}
            )
    ...
    def validate_map_data(
        ...
        try:
            ...
            return self._compile_result(
                is_valid, confidence, issues, details, validation_start
            )
    ...
    def validate_format_compatibility(
        ...
        try:
            ...
            if hasattr(source_format, 'dimensions') and hasattr(target_format, 'dimensions'):
                if source_format.dimensions != target_format.dimensions:
                    issues.append(ValidationIssue(
                        ...
                        severity='error' if abs(source_format.dimensions[0] - target_format.dimensions[0]) > 2 else 'warning',
            ...
            if hasattr(source_format, 'decimal_places') and hasattr(target_format, 'decimal_places'):
    ...
    def _validate_format_structure(self, content: str) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
        ...
        best_separator = max(separator_analysis, key=separator_analysis.get) if separator_analysis else None
    ...
    def _validate_numeric_content(self, df: pd.DataFrame) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
    ...
    def _validate_expected_format(self, content: str, expected_format: Any) -> List[ValidationIssue]:
    ...
    def _validate_expected_dimensions(self, content: str, expected_dimensions: Tuple[int, int]) -> List[ValidationIssue]:
        ...
        first_line_fields = len([f for f in lines[0].split('\t') if f.strip()])  # Assume tab separation
        ...
        if actual_dimensions != expected_dimensions:
            severity = 'error' if abs(actual_rows - expected_rows) > 2 or abs(first_line_fields - expected_cols) > 2 else 'warning'
    ...
    def _validate_ftmanager_compatibility(self, content: str, strict_mode: bool) -> List[ValidationIssue]:
    ...
    def _validate_performance(self, content: str, start_time: datetime) -> Tuple[List[ValidationIssue], Dict[str, float]]:
    ...
    def _validate_data_quality(self, df: pd.DataFrame) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
        ...
        if not numeric_data.empty:
            ...
            details['data_quality'] = {
                ...
                'value_ranges': {
                    col: {'min': float(numeric_data[col].min()), 'max': float(numeric_data[col].max())}
