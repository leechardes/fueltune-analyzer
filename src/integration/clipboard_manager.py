...
class ClipboardManager:
    ...
    def __init__(self, preferred_method: Optional[str] = None):
        ...
        logger.info(f"Clipboard manager initialized: platform={self.platform}, "
                ...
                try:
                    ...
                    if content is not None:
                        ...
                        return ClipboardResult(
                            ...
                            metadata={
                                'operation_time': operation_start.isoformat(),
                                'processing_duration_ms': (datetime.now() - operation_start).total_seconds() * 1000,
    ...
    def set_content(
        ...
        try:
            ...
            # Validate content if requested
            if validate_content:
                validation_result = self._validate_content(content, max_size_mb=50.0)  # Larger limit for setting
                if not validation_result['valid']:
                    return ClipboardResult(
                        success=False,
                        operation='set',
    ...
    def check_availability(self) -> ClipboardResult:
        ...
        try:
            ...
            return ClipboardResult(
                success=False,
                operation='check',
                errors=[f"Availability check failed: {str(e)}"]
            )
    ...
    def _select_best_method(self) -> Optional[str]:
        ...
        priority_order = ['pyperclip', 'windows_powershell', 'macos_pbcopy', 'linux_xclip', 'tkinter']
    ...
    def _validate_content(self, content: str, max_size_mb: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if content_size_mb > max_size_mb:
                errors.append(f"Content size ({content_size_mb:.1f} MB) exceeds maximum ({max_size_mb} MB)")
    ...
    def _backup_to_temp(self, content: str) -> Path:
        ...
        try:
            ...
            backup_file = temp_dir / f"ftmanager_clipboard_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
