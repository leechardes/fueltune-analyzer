...
# Utility Functions
# =============================================
# Sempre usar cores por padrão
SUPPORTS_COLOR := 1

ifeq ($(SUPPORTS_COLOR),1)
define print_info
...
docker-run: docker-build ## Executar container Docker
	...
	docker run -d \
		...
		-v $(PWD)/logs:/app/logs \
		$(PROJECT_NAME):latest
	@$(call print_success,"Container executando em http://localhost:8503")
...
docker-compose-dev: ## Executar stack completa de desenvolvimento
	...
	@$(call print_info,"FuelTune: http://localhost:8503")
...
monitor: ## Abrir ferramentas de monitoramento
	...
	@echo "Application: http://localhost:8503"
